...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    let scheduleFilter = "";
    if (subject_id) scheduleFilter = `AND sc.subject_id = '${subject_id}'`;
    ...
    const breakdown = rows.map((row) => {
      ...
      return {
        studentId: row.student_id, studentName: String(row.student_name ?? "Unknown"),
        nis: String(row.nis ?? ""), classId: row.class_id, className: String(row.class_name ?? ""),
        subjectId: row.subject_id, subjectName: String(row.subject_name ?? ""),
        present: Number(row.present), late: Number(row.late),
        absent: Number(row.absent), izin: Number(row.izin), sick: Number(row.sakit),
        total, percentage: pct,
      };
    });

    return NextResponse.json({ month, year, classId: class_id ?? null, subjectId: subject_id ?? null, totals, breakdown });
  } catch (error) {
